...
| 2026-09-24 | Мизозони беруна (`Customer`) — системаи дуюми корбар, комилан ҷудо аз `User`: ҷадвалҳои худ, JWT-схемаи `Customer` бо калиди алоҳида (`Jwt:CustomerKey`), policy `CustomerOnly`, endpoint-ҳо дар `/api/public/*`. Сабти ном бо email + коди 6-рақама (Resend, аз `office@nizom.tj`) ва Google (Apple код дорад, вале корбар онро истифода намебарад) | Корбар лоиҳаро ба SaaS мекушояд: кормандон бетағйир мемонанд, мизозон худашон сабт мешаванд. Multi-tenancy (каналҳо/автоматизатсия барои ҳар мизоз) — фазаи алоҳидаи баъдӣ; `AGENTS.md` ("multi-tenancy НЕСТ") он вақт нав мешавад |
| 2026-09-24 | Фазаи 13 (backend): обуна — 7 рӯз trial аз тасдиқи email, баъд тариф. Пардохт дастӣ: мизоз ба корт маблағи ягона (мас. 200.37) мегузаронад, чекро бор мекунад, корманд бо `subscriptions.manage` тасдиқ/рад мекунад. Ниг. `docs/phases/phase-13-subscriptions.md` | Гейтвейи онлайн нест; дирамҳои тасодуфӣ ягона роҳи ёфтани пардохтдиҳанда дар выпискаи бонк аст. Саҳифаи модератор дар office-web — баъдтар (қарори корбар) |
