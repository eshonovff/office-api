...
| 2026-09-14 | Фазаи 10: автоматизатсияи коментарии Instagram (V1) — `automation_rules`/`automation_runs`, webhook-и `entry[].changes[].field=comments` (шохаи нав дар `WebhookProcessor`, ҷудо аз `entry[].messaging[]`), `InstagramProvider.ReplyToCommentAsync`/`SendPrivateReplyAsync`/`GetRecentMediaAsync`, `CommentAutomationEndpoints` (CRUD+dry-run+instagram-media). Маҳдудиятҳои Meta тасдиқшуда (ҳуҷҷати расмӣ, 2026-09-14): private reply — як бор барои як коментарий, дар давоми 7 рӯз (на Live). Санҷидашуда зинда бо канали воқеии `eshonov.f1`: обунаи webhook (messages+comments) муваффақ шуд, филтри ҳалқа/cooldown/dry-run ҳарсе тасдиқ шуданд бо DB-и воқеӣ, ва ду дархости Graph API (ReplyToComment/SendPrivateReply) бо comment id-и синтетикӣ ба Meta расиданд ва хатогии сохториро гирифтанд (code 100 — далели дурустии endpoint/auth, на нокомии шабака) | `target_media_external_id` ба `automation_runs` илова шуд (аз рӯйхати сутунҳои спецификатсия берун) — cooldown "як actor дар як пост" бе media id физикӣ ғайриимкон буд. Тафсил: `docs/phases/phase-10-instagram-automation.md` |
| 2026-09-14 | Дар ҳамон рӯз: тугмаи DM (`dmButtonUrl`) воқеан татбиқ шуд — Фазаи 10 онро танҳо захира мекард, дар `SendPrivateReplyAsync` ҳеҷ гоҳ истифода намешуд (DM ҳамеша матни оддӣ мерафт). `DmButtonTitle` (то 20 ҳарф — маҳдудияти Messenger button template) илова шуд; агар URL+сарлавҳа дода шаванд, паём ҳамчун button template (на матни оддӣ) меравад. Санҷидашуда зинда: Meta шакли payload-ро қабул кард | Кашфшуда аз саволи корбар "ин барои чист" — далели он, ки як майдон дар UI/DB буданаш маънои истифода шуданашро намедиҳад |
| 2026-09-14 | Фазаи 11: тасдиқи обуна (V2) — `condition_config.requiresFollow`, `action_config.{onMatch,onNotFollowing}`, `InstagramProvider.CheckFollowStatusAsync` (`GET /{user-id}?fields=is_user_follow_business`, кэши 15-дақ дар `IMemoryCache`, ҳеҷ гоҳ истисно намепартояд — Unknown fail-open), `AutomationBranchSelector` (pure). Санҷидашуда зинда бо канали воқеӣ: `Following` (actor-и воқеӣ), `Unknown` (actor синтетикӣ), кэш (дархости дуюм 0.01с, СИФР дархости нави Graph API дар лог) | Миграцияи `AddFollowCheckCondition` action_config_json-и Фазаи 10-и ҳамвор (rule-и зиндаи "Faridun" дар DB) ба шакли `{OnMatch,OnNotFollowing}` бо SQL-и idempotent табдил дод — бе ин rule дар аввалин иҷрои баъдӣ NullReferenceException медод. Тафсил: `docs/phases/phase-11-follow-check.md` |
| 2026-09-15 | Ду масъала аз санҷиши зиндаи корбар: (А1) DM акнун ихтиёрӣ — `DmText` холӣ → `DmStatus=Disabled`, ҳеҷ дархост ба Meta намеравад (корбар метавонад танҳо ба коментарий ҷавоб диҳад). (А2) `FollowCheckResult.NotFollowing` дигар КЭШ НАМЕШАВАД (Following/Unknown ҳамоно кэш мешаванд) — санҷидашуда зинда: пеш аз ин, корбаре ки обуна шуд, то 15 дақ боз ҷавоби "обуна нест" мегирифт. Ба ҷои кэши NotFollowing: идемпотентии comment_id дар `CommentAutomationProcessor` (санҷидашуда зинда: 2 webhook-и якхела → 1 run) + буҷаи соатии 80% (`InstagramFollowCheckRateLimiter`, 160/200) | Корбар худаш ин ду масъаларо тавассути санҷиши воқеӣ ёфт, на тавассути тест — тарҳи ҳалли А2-ро худи корбар пешниҳод кард (кэши NotFollowing "корбареро, ки дуруст кор кард, ҷазо медиҳад"). 12 тести нав. Тафсил: `docs/phases/phase-11-follow-check.md` (бахши "Навсозӣ") |
| 2026-09-15 | Фазаи 12 (backend): Flow Builder — граф аз нодҳо (`flows`/`flow_nodes`/`flow_edges`), сессияи иҷро бо ҳолати доимӣ (`flow_sessions`/`flow_session_steps`), `contact_tags`/`contact_variables`, `FlowEngine` (480 сатр — старт/шоха/тугма/delay/collect-input/window-closed/goto_flow/http_request), `FlowTriggerProcessor` (паҳлӯи `CommentAutomationProcessor`-и мавҷуда), `InstagramProvider.SendButtonMessageAsync`, postback parsing, 3 шаблони аввалия, `GET /api/automations` (union бо `automation_rules`-и бетаъсирмонда). Санҷидашуда зинда бо канали воқеӣ: webhook→trigger→session→нодаи 1 (паёми воқеӣ, Meta 200 OK)→waiting дар нодаи 2 (collect_input) — бе хатогӣ. 46 тести нав (598 ҷамъ) | Frontend (canvas, `@xyflow/react`, ~60%-и фаза тибқи спека) ҳанӯз ОҒОЗ НАШУДААСТ — қасдан, тибқи тартиби "backend+санҷиш+ҳуҷҷат аввал". Тафсил: `docs/phases/phase-12-flow-builder.md` |
| 2026-09-15 | Фазаи 12 (frontend, ҳамон рӯз): `/automations` + canvas (`office-web`, ниг. `office-web/docs/phases/phase-9-flow-builder-ui.md`) пурра иҷро шуд. Дар доираи он ду майдони хурд ба backend илова шуд: `FlowListItem`/`FlowDetail.ChannelId` (canvas-ро лозим буд, ҳеҷ endpoint нашикаст, 598 тест ҳамоно сабз) | Санҷиши дастии зиндаи browser (frontend) иҷро нашуд — абзори browser дар сессия дастрас набуд |
| 2026-09-24 | Мизоҷони беруна (`Customer`) — системаи дуюми корбар, комилан ҷудо аз `User`: ҷадвалҳои худ, JWT-схемаи `Customer` бо калиди алоҳида (`Jwt:CustomerKey`), policy `CustomerOnly`, endpoint-ҳо дар `/api/public/*`. Сабти ном бо email + коди 6-рақама (Resend, аз `office@nizom.tj`) ва Google (Apple код дорад, вале корбар онро истифода намебарад) | Корбар лоиҳаро ба SaaS мекушояд: кормандон бетағйир мемонанд, мизоҷон худашон сабт мешаванд. Multi-tenancy (каналҳо/автоматизатсия барои ҳар мизоҷ) — фазаи алоҳидаи баъдӣ; `AGENTS.md` ("multi-tenancy НЕСТ") он вақт нав мешавад |
| 2026-09-24 | Фазаи 13 (backend): обуна — 7 рӯз trial аз тасдиқи email, баъд тариф. Пардохт дастӣ: мизоҷ ба корт маблағи ягона (мас. 200.37) мегузаронад, чекро бор мекунад, корманд бо `subscriptions.manage` тасдиқ/рад мекунад. Ниг. `docs/phases/phase-13-subscriptions.md` | Гейтвейи онлайн нест; дирамҳои тасодуфӣ ягона роҳи ёфтани пардохтдиҳанда дар выпискаи бонк аст. Саҳифаи модератор дар office-web — баъдтар (қарори корбар) |
| 2026-09-25 | Фазаи 16: шарҳҳои Instagram — ҷавоби оммавӣ дар flow (вариантҳо бо навбат, таъхири 5–20 с) ва саҳифаи `/account/comments` (ҷавоб, як Direct, пинҳон, нест, sync, realtime). Дар рафти кор ислоҳи амниятӣ: webhook-и Meta бо `entry[]`-и чанд аккаунт акнун ҳар `entry`-ро бо канали худаш коркард мекунад (пеш ҳама бо канали `entry[0]` — шарҳи мизоҷи B метавонист ба A равад); `webhook_logs` бо нест кардани ҳисоб нест мешаванд | Шарҳҳо дар база нигоҳ дошта мешаванд (`instagram_comments`, филтри tenant), на ҳар бор аз Graph API — барои «нав», филтрҳо ва realtime. Тафсил: `docs/phases/phase-16-comments.md` |
