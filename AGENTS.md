...
Усули стандартӣ — `./dev.sh` дар решаи `office-api/`, на `dotnet run`-и мустақим:

```
./dev.sh
```

Он Docker-ро месанҷад, `docker compose up -d` мекунад, интизори омода шудани
PostgreSQL мешавад (то 30 сония) ва баъд `dotnet run`-ро дар `Office.Api/`
сар медиҳад.
...
9. Оғози сервери локалӣ — танҳо `./dev.sh`, на `dotnet run`-и мустақим.
10. Агар дар вақти санҷиш `docker compose down` кардӣ, дар охири кор ҳатман
    `docker compose up -d` кун — базаро холӣ нагузор.
